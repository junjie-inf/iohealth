		...
		<tr>
			<td>
				<p style="font-size:17px;margin-bottom:10px;font-weight:normal;font-size:14px;line-height:1.6;">
					El usuario <a href="{{ URL::route('user.show', $model->user->id) }}">{{ $model->user->getFullname() }}</a> ha creado un nuevo informe:
				</p>
				<p style="font-size:14px;margin-bottom:10px;font-weight:normal;line-height:1.6;">
					<a href="{{ URL::route('report.show', $model->id) }}"><strong>{{ $model->title }}</strong></a>
				</p>
			</td>
		</tr>
	</table>
