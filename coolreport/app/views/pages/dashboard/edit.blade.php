...
@section('title', trans('sections.dashboard.edit'))
	...
	<div class="col-lg-12">
		<div class="box">
			<div class="box-header">
				<h2><i class="icon-plus"></i><span class="break"></span>{{ trans('sections.dashboard.edit') }}</h2>
				<div class="box-icon">
					<a href="{{ URL::route('dashboard.index') }}"><i class="icon-arrow-left"></i><span>{{ trans('dashboard.back') }}</span></a>
				</div>
			</div>
			<div class="box-content">
				<form class="form-horizontal form-dashboard-update fix-margins" action="" data-type="template" data-id="{{ $dashboard->id }}">
					<fieldset class="col-sm-12">
						
						<div class="form-group">
							<label class="control-label" for="title">{{ trans('dashboard.title') }}</label>
							<div class="controls">
								<input class="form-control" id="title" name="title" type="text" value="{{ $dashboard->title }}" required autofocus />
							</div>
						</div>
...
$(document).ready(function() {
	Dashboards.fromJson({{ json_encode($dashboard->datum->items) }});
});
</script>
@stop
