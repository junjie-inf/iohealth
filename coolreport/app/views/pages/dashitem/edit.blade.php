...
@section('title', trans('sections.dashitem.edit'))
	...
	<div class="col-lg-12">
		<div class="box">
			<div class="box-header">
				<h2><i class="icon-plus"></i><span class="break"></span>{{ trans('sections.dashitem.edit') }}</h2>
				<div class="box-icon">
			...
			<div class="box-content">
				...
				<div class="step-content">
									...
									<div class="controls">
										<select id="template-select" name="template" class="form-control" data-rel="chosen" data-placeholder={{ trans('dashitems.chooseatemplate') }}>
											@foreach( $templates as $t )
												<option value="{{ $t->id }}"
													@if( $t->id == $dashitem->datum->data->from[0]->template)
													selected
													@endif
													>{{{ $t->title }}}</option>
											@endforeach
					...
					<div id="step-data" class="step-pane">
						...
						<form class="form-horizontal form-dashboard-update fix-margins row" action="" data-type="template" data-id="{{ $dashitem->id }}">					
										...
										<div class="btn-group">
											...
											<div class="btn-group">
												...
												<ul class="dropdown-menu" id="db-add-aggregate-dropdown">
													...
													<li><a href="javascript:void(0)" class="action-add-aggregate" data-type="sum"><i class="icon-plus"></i> {{ trans('dashitems.sum') }}</a></li>
							...
							<div class="row">
								<div class="col-sm-12 controls">
									<div class="input-group input-group-sm">
										...
										<input class="form-control cr-filter-condition" value="{{ $dashitem->datum->data->filter }}" placeholder={{ trans('dashitems.clicktoeditexpression') }}>
									</div>
								</div>
							</div>
							</fieldset>
					...
					<div id="step-preview" class="step-pane">
						...
						<div class="form-group">
								...
								<div class="controls">
									<input class="form-control" id="title" name="title" type="text" value="{{ $dashitem->title }}" required autofocus />
								</div>
						</div>
...
$(document).ready(function(){
	...
	$('#dashboard-wizard')
		.on('change', function(ev, d)
		{
			...
			if (wizardForward && d.step == 4 && !DashItems.validateTitle())
			{
				ev.preventDefault();
			}
		})
		...
		.on('finished', DashItems.update);
	
	if ('1' == '{{ (isset($dashitem->datum->political) && $dashitem->datum->political) }}')
	{
		$('#political-type').trigger('click');
	}
	else
	{
		if ('1' == '{{$dashitem->datum->display->type == 'map'}}')
		{
				$('#map-type').trigger('click');
		}
		else
		{
				$('#chart-type').trigger('click');
		}
	}

	$('#dashboard-wizard').wizard().data('wizard').next();
	//Initialize data from JSON
	DashItems.fromJson(
		{{ json_encode($dashitem->datum->data) }}
	);
	...
	DashDisplay.fromJson( {{ json_encode($dashitem->datum->display) }} );
});
</script>
@stop
